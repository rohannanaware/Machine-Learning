...
# list of updates
# working dir
# generate packing dim constraints code
# lbh sorting
# increment
# =========================================================
                                     ...
                                     packing_box_data=packing_box_data) 
{
  ...
  sorted_df <- t(apply(processed_packing_data[,c('wid_l_with_tol','wid_b_with_tol','wid_h_with_tol')], 
                       1,
                       FUN = function(x) sort(x, decreasing = T)))
  
  sorted_df <- data.table(sorted_df)

  processed_packing_data$sorted_l_with_tol <- sorted_df$wid_l_with_tol
  processed_packing_data$sorted_b_with_tol <- sorted_df$wid_b_with_tol
  processed_packing_data$sorted_h_with_tol <- sorted_df$wid_h_with_tol
...
generate_packing_boxes <- function(lmin=lmin,bmin=bmin,hmin=hmin,lmidmin=lmidmin,bmidmin=bmidmin,hmidmin=hmidmin,lmax=lmax,bmax=bmax,hmax=hmax,increment=0.6){# change increment due to compute constraints
...
#     }
#   }
#   # taking too long to run - cross join and apply the lbh difference condition
#   
...
# =========================================================
load_and_process_raw_data <- function(working_dir="C:/Users/rohan.nanaware/Documents/03 packaging optimization/06 BBD 2020 data run",
  ...
  if(file.exists(new_packing_boxes_data_file_name) & file.exists(packing_dim_constraints_data_file_name) ){
    ...
    new_packing_boxes <- generate_packing_boxes(increment=0.6,
                                                lmin=lmin,lmax=lmax,
                                                bmin=bmin,bmax=bmax,
                                                hmin=hmin,hmax=hmax,
...
run_or_model <- function(initial_boxes_selected=initial_boxes_selected,
  ...
  ifelse(readline('Run the optimization code from Spyder and press 1 and enter once run is complete :)') == 1,"Thank you!","Meh :(")
  # python_run_status <- system2('C:/Users/rohan.nanaware/AppData/Local/Continuum/anaconda3/python',as.character(c('pack_gurobi.py',no_of_boxes,no_of_existing_boxes)),stdout = T,stderr = T)
...
optimize_packing_boxes <- function(working_dir="C:/Users/rohan.nanaware/Documents/03 packaging optimization/06 BBD 2020 data run",
                                   training_file ="packing_raw_mobiles.csv",
                                   test_file="packing_raw.csv",
                                   packing_box_file='packing_boxes_all.csv',
                                   cms_vertical='non_mobiles',
                                   lmin=6.0,
                                   bmin=3.5,# reduced from 4.5 to 3.5 due to ibl size drop
                                   hmin=1.0,
                                   lmidmin = 0,
                                   bmidmin = 0,
                                   hmidmin = 0,
